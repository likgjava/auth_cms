...
<script src="${initPath}/view/auth/portal/front.js"></script>
<script>
//如果是在Frame中打开此页面，则重新在浏览器中打开登录页面（避免在后台操作，当session失效时，在Frame中打开登录页面）
function escapeFrame(){
	if(window.top.location.href != window.location.href){
		window.top.location.href = '/view/auth/login/login.jsp';
	}
}
			...
			success: function(json){
				if(json.success){
					...
					if(json.isAdmin == 1){
						...
						window.location.href = $('#initPath').val()+'/'; //跳转到首页
					}
				}else{
            ...
            <td>
				...
				<table width="96%" border="0" align="center" cellpadding="0" cellspacing="5">
					...
					<tr>
						...
						<td colspan="2"><input type="text" id="userName" name="userName" class="input required" value="admin"/></td>
					</tr>
					<tr>
						<td height="30" align="right">密　码：</td>
						<td colspan="2"><input type="password" id="password" name="password" class="input required" value="1"/></td>
					</tr>
					<tr>
