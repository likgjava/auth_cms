...
<form id="menuForm" action="/MenuController.do?method=save" method="post">
...
$(document).ready(function() {
	...
	$('#resourceName').click(function(){
		new $.msgbox({
			...
			height: 400,
			type: 'ajax',
			content: '/view/auth/common/tree/DHtmlTree.jsp?className=Resource&IDS=resourceId&NAMES=resourceName',
			onAjaxed: function(){
			...
			success: function(json){
				if(json.success){
					...
					if(menuObjId){
						...
						//刷新表单域
						$('#menuInfo').load('/MenuController.do?method=toMenuDetailView', {objId: menuObjId});
					}
					//新增
					else{
						//刷新树节点
						...
						//刷新表单域
						$('#menuInfo').load('/MenuController.do?method=toMenuDetailView', {objId: MenuTree.getSelectedItemId()});
					}
				}else{
					alert(json.result);
				}
